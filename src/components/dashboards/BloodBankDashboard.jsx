...
const BloodBankDashboard = ({ user, banks, setBanks, requests, setRequests, onLogout }) => {
  const bank    = banks.find((b) => b.username === user.username) || banks[0];
  const [editing, setEditing] = useState(false);
  const [inv, setInv]         = useState({ ...bank.inventory });

  const pending = requests.filter((r) => r.bankId === bank.id && r.status === "pending");

  const saveInventory = () => {
    setBanks((bs) =>
      bs.map((b) => (b.id === bank.id ? { ...b, inventory: { ...inv } } : b))
    );
    setEditing(false);
  };
  ...
  const handleRequest = (reqId, action) => {
    ...
    if (action === "approve") {
      setBanks((bs) =>
        bs.map((b) => {
          if (b.id === bank.id) {
            const newInv = { ...b.inventory };
            newInv[req.bloodType] = Math.max(0, (newInv[req.bloodType] || 0) - req.units);
      ...
      setInv((i) => ({
        ...i,
        [req.bloodType]: Math.max(0, (i[req.bloodType] || 0) - req.units),
      }));
    }
    setRequests((rs) =>
      rs.map((r) =>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              {editing ? (
                <div style={{ display: "flex", gap: 8 }}>
                  <Btn size="sm" outline onClick={() => setEditing(false)}>Cancel</Btn>
                  <Btn size="sm" onClick={saveInventory}>Save</Btn>
                </div>
              ) : (
              ...
              {BLOOD_TYPES.map((bt) =>
                editing ? (
                  <div
                    key={bt}
                    style={{
                      background: S.redPale, borderRadius: 10, padding: 12,
                  ...
                  >
                    ...
                    <input
                      ...
                      value={inv[bt] || 0}
                      onChange={(e) =>
                        setInv((i) => ({ ...i, [bt]: parseInt(e.target.value) || 0 }))
                      }
                      style={{
                        width: "100%", boxSizing: "border-box",
                        border: `1px solid ${S.redBorder}`, borderRadius: 7,
                  ...
                  <BloodBadge key={bt} type={bt} units={bank.inventory[bt] || 0} />
                )
              )}
            </div>
          </Card>
          ...
          <Card>
            ...
            {pending.length === 0 ? (
              ...
              pending.map((r) => (
                <div
                  key={r.id}
                  style={{ border: `1px solid ${S.border}`, borderRadius: 10, padding: 14, marginBottom: 10 }}
                >
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontWeight: 700 }}>{r.hospitalName}</span>
                    <span
                      style={{
                        background: r.urgency === "Critical" ? "#FFF0F0" : "#FFF8E7",
                        color: r.urgency === "Critical" ? S.urgent : S.warning,
                  ...
                  <p style={{ margin: "4px 0 10px", fontSize: 13, color: S.muted }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <Btn
                      ...
                      style={{ flex: 1, background: S.success }}
                    >
                      Approve
                    </Btn>
                    <Btn
